...
Model.prototype.init = function(done) {
	...
	async.waterfall([
		function(next) {
			model.db.run('drop table if exists transitions', next)
		},
		function(next) {
			model.db.run('create table transitions(txn_id integer, sequence text)', next)
...
Model.prototype.getTransitions = function(done) {
	...
	async.waterfall([
		function(next) {
			model.db.all('select * from transitions', next)
		},
		function(rows, next) {
